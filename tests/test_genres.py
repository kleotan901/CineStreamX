...
from tests.helper import (
    ...
    override_require_moderator_with_unauthenticated_exception,
)
...
@pytest.mark.asyncio
async def test_get_genres_with_movies_count(db_session, seed_movie_certification):
    """
...
async def test_update_genre_by_moderator(db_session, seed_genres):
    ...
    assert (
        updated_genre.name == "Comedy"
    ), "The genre's name in DB should be changed from 'Genre-2' to 'Comedy'"
